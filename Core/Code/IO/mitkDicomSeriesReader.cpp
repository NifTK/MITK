...
void DicomSeriesReader::ExtractAllTags(DcmIoType * dicomIO, mitk::Image * image)
{
  ...
  mitk::StringTaggedStringLookupTable dcmTagsMap;
  ...
  while (itr != end)
  {
    ...
    if (entryvalue)
    {
      ...
      if (found)
      {
        ...
        dcmTagsMap.SetTableValue(tagkey, stringVal);
      }
      else
      {
  ...
  mitk::StringTaggedStringLookupTableProperty::Pointer dicomTags = mitk::StringTaggedStringLookupTableProperty::New();
  dicomTags->SetValue(dcmTagsMap);
