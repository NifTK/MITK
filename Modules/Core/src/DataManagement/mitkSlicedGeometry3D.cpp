...
void
  ...
  mitk::PlaneGeometry* geometry2D, unsigned int slices )
{
  assert( geometry2D != nullptr );
  ...
  bool top, bool frontside, bool rotated )
{
  ...
  /// well, just it has to be consistent, e.g. the same viewer should not have
  /// renderers with left- and right-handed geometries at the same time.
  /// I leave the test here for debugging purposes, but comment it out so that
  /// people can initialise renderers with left-handed coordinate system.
